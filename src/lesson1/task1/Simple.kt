...
fun lengthInMeters(sagenes: Int, arshins: Int, vershoks: Int): Double {
    val vershoks = 0.04445 * vershoks
    val arshins = 0.7112 * arshins
    val sagenes = 2.1336 * sagenes
    return sagenes + arshins + vershoks
}
...
fun angleInRadian(deg: Int, min: Int, sec: Int): Double {
    val deg = deg * PI / 180
    val min = min * PI / (60 * 180)
    val sec = sec * PI / (180 * 3600)
    return deg + min + sec
}
...
fun accountInThreeYears(initial: Int, percent: Int): Double {
    val initial = initial
    val percent = percent * 0.01
    return initial * ((1.0 + percent) * (1.0 + percent) * (1.0 + percent))
}
